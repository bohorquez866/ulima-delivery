<template>
  <div class="sup_box" id="container">
    <form @submit.prevent="login" class="form_box">
      ...
      <p>
        No tienes cuenta,
        <router-link :to="{ name: 'Register' }" class="register_now">Registrate ahora</router-link>
      </p>

      <div class="check-terms">
        <input type="radio" name="terms and conditions" id="" required="true" class="check" />
    ...
    async login() {
      ...
      setPersistence(auth, browserLocalPersistence).then(() => {
        signInWithEmailAndPassword(auth, this.email, this.password)
          .then(() => {
            this.$router.replace({ name: "InstruccionesPage" });
          })
          .catch((error) => {
            switch (error.code) {
